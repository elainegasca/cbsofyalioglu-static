...
export function _ListItemCard(props: CardProps) {
  return (
    <li
      title={props.title}
      className={`group h-40 md:h-64 xl:h-52 flex flex-col  rounded-lg shadow-lg overflow-hidden relative mt-4${props.className ? ` ${props.className}` : ''
        }`}
    >
      <Image
        loading="lazy"
