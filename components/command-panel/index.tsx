...
import { useHasMounted, useScrollLock, useOnClickOutside } from '../../lib/hooks'
...
export function Commander({ data, open, closeHandler }) {
  const inputRef = React.useRef<HTMLInputElement | null>(null)
  const ref = React.useRef(null)

  useOnClickOutside(ref, closeHandler)

  useScrollLock(open ? true : false)
  const [search, setSearch] = React.useState('')
  const listRef = React.useRef(null)

  const english = data?.filter((p) => p.language === 'en' || p.language === 'eng')
  const turkish = data?.filter((p) => p.language === 'tr')
  React.useEffect(() => {
  ...
  return (
    <div className="raycast commander" style={{ top: window.scrollY }}>
      <Command layoutId="cp" value={search} onValueChange={(v) => setSearch(v)} ref={ref}>
        <div cmdk-raycast-top-shine="" />
        <Command.Input ref={inputRef} autoFocus placeholder="Search" />
        <hr cmdk-raycast-loader="" />
        <Command.List ref={listRef}>
          <Command.Empty>No results found.</Command.Empty>
          <Command.Group heading="Articles in English">
            {english.map((p) => (
              <Command.Item>
                <Link href={`/${p.topic}/${p.slug}`} className="flex">
                  <img src={p.cover} className="rounded-full w-10 h-8 mr-4" />{' '}
                  {p.title}
                </Link>
              </Command.Item>
            ))}
          </Command.Group>
          <Command.Separator style={{ marginTop: 16 }} />
          <Command.Group heading="Türkçe Yazılar">
            {turkish.map((p) => (
              <Command.Item>
                <Link href={`/${p.topic}/${p.slug}`} className="flex">
                  <img src={p.cover} className="rounded-full w-10 h-8 mr-4" />{' '}
        ...
        <div cmdk-raycast-footer="">
          <button cmdk-raycast-open-trigger="" onClick={closeHandler}>
            Close
            <kbd>ESC</kbd>
          </button>
