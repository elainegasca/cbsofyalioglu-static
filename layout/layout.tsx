    ...
    return (
        <Accordion type="single" collapsible>
            {navigation.map((item, i) =>
                item.children ? (
                    <AccordionItem value={`item-${i}`} key={`acc-desktop-${i}-${item.name}`}>
                        <AccordionTrigger
                        ...
                        >
                            {item.icon ? (
                                <span className="flex items-center py-4 opacity-90 hover:opacity-100 transition-opacity duration-200 ease-linear">
                                    <item.icon />{' '}
                                    <span
                        ...
                        <AccordionContent>
                            {item.children.map((subItem, i) => (
                                <motion.a
                                    key={`acc-desk-${subItem.name}-${i}`}
                                    href={subItem.href}
                                    className="group w-full flex items-center pl-5 pr-2 py-2 text-sm font-medium  rounded-md"
                                    title={subItem.name}
                                    style={{
                                        color:
                                            item.color && subItem.current
                                                ? item.color
                                                : 'rgba(200, 200, 200, 0.8)',
                                    }}
                                    whileHover={{
                                        color: item.color ? item.color : 'rgba(200, 200, 200, 1)',
                                    }}
                                >
                                    {subItem.name}
                                </motion.a>
                    ...
                    <motion.a
                        key={`sidebar-desktop-single-${item.name}-${i}`}
                        href={item.href}
                        title={item.name}
                    ...
                    >
                        {item.icon ? (
                            <span className="flex items-center opacity-90 hover:opacity-100 ease-linear transition-opacity">
                                <item.icon />{' '}
                                <span
                                    className="ml-2 text-2xl"
                                    style={{ color: item.color ? item.color : '#ccc' }}
...
const _SidebarDesktop = ({ openCommander }) => (
    <div
        id="sidebar-desktop"
        className="hidden relative sm:flex md:flex-shrink-0  z-10 border-r-2 border-solid border-gray-900"
    >
        <div className="flex flex-col w-80">
            ...
            <div
                id="sidebar-fixed"
                className="flex flex-col h-0 flex-1 fixed top-16 left-0 w-64 bottom-0 max-h-screen"
            >
                {/* SIDEBAR TOP BANNER*/}
                <div
                    title={'cbsofyalioglu'}
            ...
            <a className="relative top-2 uppercase font-black text-lg" title="homepage">
              ...
              cbs<span className="!text-4xl !font-black !leading-[16px] !h-[16px] !text-[#e5ff01]">.</span>
                ...
                <div className="flex items-center h-16 flex-shrink-0 px-4 bg-transparent justify-between"></div>
